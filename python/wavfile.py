...
def _read_fmt_chunk(fid):
    res = struct.unpack('<ihHIIHH', fid.read_wave(20))
    ...
    if (comp != 1 or size > 16):
        ...
        if (size>16):
            fid.read_wave(size - 16)
    return size, comp, noc, rate, sbytes, ba, bits

# assumes file pointer is immediately
#   after the 'data' id
def _read_data_chunk(fid, noc, bits, normalized=False):
    size = struct.unpack('<i', fid.read_wave(4))[0]
...
def _read_unknown_chunk(fid, name):
    ...
    if bool(size & 1):     # if odd number of bytes, move 1 byte further (data chunk is word-aligned)
      offset = 1
    string = fid.read_wave(size)
    fid.seek(offset, 1)
    return string

def _read_riff_chunk(fid):
    str1 = fid.read_wave(4)
    if str1 != b'RIFF':
        raise ValueError("Not a WAV file.")
    fsize = struct.unpack('<I', fid.read_wave(4))[0] + 8
    str2 = fid.read_wave(4)
    if (str2 != b'WAVE'):
        raise ValueError("Not a WAV file.")
    return fsize
